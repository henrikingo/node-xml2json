...
struct cResourceBuilder : cScr
{
    ...
    o3_fun void buildAndAppend(iFs* target_file, siEx* )
    {
        ...
        offset = target_file->size() + sizeof(size_t) + size_of_header;
        ...
        size_t orig_size = target_file->size();
    ...
    bool addDir(const char* path, iFs* dir) 
    {
        ...
        Str new_path(path);
        if (!path || *path)            
            new_path.append( dir->name());
        
        bool success = true;

        tVec<siFs> children = dir->children();
        for (size_t i=0; i<children.size(); i++){
            if (children[i].ptr()->isFile())
                success = addFile(new_path, children[i]);
            else if (children[i].ptr()->isDir())
