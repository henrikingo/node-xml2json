...
//#endif
 ...
 //   }
    ...
    siModule loadModule(const char* name)
    {
		...
		Str prefix, libname(name);

#ifdef O3_PLUGIN
		prefix.append(Str(getenv("HOME")) + "/.o3/" + O3_VERSION_STRING + "/components/");
		libname.append(O3_VERSION_STRING);
		prefix.append("lib");	
#else
		prefix.append("/usr/lib/o3/lib");
#endif
        ...
        libname.append(".so");
#endif // O3_LINUX
        handle = dlopen(prefix + libname, RTLD_NOW);
		if (!handle) 
            return siModule();
...
o3_trace_sys("approvalBox");
		...
		MessageBox( char* header, char* msg, unsigned long msg_type )
		{
			...
			CFStringRef messaage_ref 
				= CFStringCreateWithCString( NULL, msg,  strlen(msg) );
