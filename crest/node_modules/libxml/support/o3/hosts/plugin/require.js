...
(function () {
    ...
    req.exec = function (module, traced, waiting, context, reenter) {
        ...
        if (!name && !reenter && befCbs && !this.s.plugins.wasAlreadyCalled) { //Finall callback            
            var ii = -1, l = befCbs.length;
			var f = function(){				
                while (!befCbs[++ii] && ii != l) {}
                if (ii == l)
                    req.exec(module, traced, waiting, context, true);
                else if (befCbs[ii])
                    befCbs[ii].call(require, f, context);
            };
			this.s.plugins.wasAlreadyCalled = true;
            return f();
        }
