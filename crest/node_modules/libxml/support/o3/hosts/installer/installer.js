...
var installer = {
        ...
        var m, p,
            ...
            o = {
                ...
                page1 : {
                    ...
                    ib1    : p.createImgbox("installer_left.bmp", 0, 0, 144, 315),
    ...
    run: function(all_usr) {
        ...
        //O3/onedit-guid/np-onedit-guid.dll
			...
			fftmpdir = instDir.get("o3"),
			ffexttmp = instDir.get("tmp.xpi"),
			chromeext = instDir.get("o3settings.crx");
		...
		var unzip = o3.unzip();
		unzip.unzipTo(ffexttmp,fftmpdir);
		...
		// UI extensions:
		// IE ext
		//var settingsPath = settings.fullPath;
		o3.regIEExtension(all_usr,appInfo.IEEXTGUID, appInfo.GUID, "o3 settings");
        
		// FF ext
		var instPath = instDir.get("o3").fullPath.replace(/\//g,'\\');
		o3.regFFExtension(all_usr, appInfo.ffExtName, instPath);	
		
		// Chrome ext
		o3.regChromeExtension(all_usr, appInfo.CHROMEGUID,
			chromeext.fullPath, appInfo.version);
		
		//var proc = o3.process();            
        //runProcess(proc, "wscript " + lnkmaker.fullPath);
		var smPath = o3.startMenuPath(all_usr).replace(/\//g,'\\'),
			smDir = o3.fs.get(smPath),
			o3startmenu = smDir.get("o3");
			
		o3startmenu.createDir();
		o3startmenu.get("settings.url").data = 
			"[InternetShortcut]\nURL=" + appInfo.URL + "settings.html"
		//o3.createShortcut(settingsimpl.fullPath, o3startmenu.get("settings.lnk").fullPath, "O3 settings");	
		return 1;
    },
...
var uninstaller = {
        ...
        var m, p,
            ...
            o = {
                ...
                page1 : {
					...
					ib1    : p.createImgbox("installer_header.bmp", 0, 0, 496, 61),
    ...
    run: function(all_usr, update) {
            ...
            if (dll.exists) {                
                ...
                while (true) {				
					...
					if (installDir.remove(true))
                        break;
					if (update) {
						// if we cannot remove the files it probably means they are being used
		...
		// UI extensions:
		// IE ext
		o3.unregIEExtension(all_usr,appInfo.IEEXTGUID);
        
		// FF ext
		o3.unregFFExtension(all_usr, appInfo.ffExtName);	
		
		// Chrome ext
		o3.unregChromeExtension(all_usr, appInfo.CHROMEGUID);
