...
METHODDEF(boolean)
jchuff_encode_mcu_DC_first (j_compress_ptr cinfo, JBLOCKROW *MCU_data)
{
...
METHODDEF(boolean)
jchuff_encode_mcu_AC_first (j_compress_ptr cinfo, JBLOCKROW *MCU_data)
{
  huff_entropy_ptr entropy = (huff_entropy_ptr) cinfo->entropy;
  register int temp, temp2;
  register int nbits;
...
METHODDEF(boolean)
jchuff_encode_mcu_DC_refine (j_compress_ptr cinfo, JBLOCKROW *MCU_data)
{
  huff_entropy_ptr entropy = (huff_entropy_ptr) cinfo->entropy;
  register int temp;
  int blkn;
...
METHODDEF(boolean)
jchuff_encode_mcu_AC_refine (j_compress_ptr cinfo, JBLOCKROW *MCU_data)
{
  huff_entropy_ptr entropy = (huff_entropy_ptr) cinfo->entropy;
  register int temp;
  register int r, k;
...
start_pass_huff (j_compress_ptr cinfo, boolean gather_statistics)
{
  ...
  if (cinfo->progressive_mode) {
    ...
    if (cinfo->Ah == 0) {
      if (cinfo->Ss == 0)
	entropy->pub.encode_mcu = jchuff_encode_mcu_DC_first;
      else
	entropy->pub.encode_mcu = jchuff_encode_mcu_AC_first;
    } else {
      if (cinfo->Ss == 0)
	entropy->pub.encode_mcu = jchuff_encode_mcu_DC_refine;
      else {
	entropy->pub.encode_mcu = jchuff_encode_mcu_AC_refine;
