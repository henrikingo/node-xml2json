...
void Agg2D::render(bool fillColor)
{
    if(m_blendMode == BlendAlpha)
    {
		...
		{
			Agg2DRenderer::render(*this, m_renBaseAlpha, m_renAlpha_Solid, fillColor);
		}
		else
		{
			Agg2DRenderer::render(*this, m_renBase, m_ren_Solid, fillColor);
		}
    }
    else
    {
		if (m_alphamaskenabled)
		{
			Agg2DRenderer::render(*this, m_renBaseAlpha_Comp, m_renAlpha_SolidComp, fillColor);
		}
		else
		{
			Agg2DRenderer::render(*this, m_renBase_Comp, m_ren_SolidComp, fillColor);
		};
    }
}
...
void Agg2D::render(FontRasterizer& ras, FontScanline& sl)
{
    if(m_blendMode == BlendAlpha)
    {
		if (m_alphamaskenabled)
		{
			Agg2DRenderer::render(*this, m_renBaseAlpha, m_renAlpha_Solid, ras, sl);
		}
		else
		{
			Agg2DRenderer::render(*this, m_renBase, m_ren_Solid, ras, sl);
		};
    }
    else
    {
		if (m_alphamaskenabled)
		{
			Agg2DRenderer::render(*this, m_renBaseAlpha_Comp, m_renAlpha_SolidComp, ras, sl);
		}
		else
		{
			Agg2DRenderer::render(*this, m_renBase_Comp, m_ren_SolidComp, ras, sl);
		}
...
void Agg2D::BuildAlphaMaskFromPath()
{
/*	m_rasterizer.add_path(m_pathTransform);
    renSolid.color(fillColor ? gr.m_fillColor : gr.m_lineColor);
    agg::render_scanlines(gr.m_rasterizer, gr.m_scanline, renSolid);
...
agg::rendering_buffer *Agg2D::GetAlphaBuffer()
{
	return &m_rbuf_alpha;
};
