...
/**
 * Experimental XML API by Henrik
 */
var xmlApi = require('./xmlapi/xmlapi.js');
...
var insertAll = function insertAll (self, docs, options, callback) {
  // xmlApi: if given xml, transparently convert to js object and continue as normal
  // bonus: as a side effect, it would be valid to mix it up: [ "<hello>world</hello>", { hello : "world" } ]
  for(var index = 0, len = docs.length; index < len; ++index) {
    var dom = xmlApi.isXmlString( docs[index] );
    if( dom ) {
      docs[index] = xmlApi.domToObj( dom );
    }
  }
...
Collection.prototype.find = function find () {
  ...
  // xmlApi: if given a query that looks like XPATH, we run a magic regex to convert it to real MongoDB query.
  // Only works for simple /a/b or /a/b[@foo] queries. parent: and sibling: and such don't make sense for MongodB.
  var xmlOut = false;
  if ( xmlApi.isXpathString( selector ) ){
     selector = xmlApi.xpathToObj( selector );
     xmlOut = true;
  }
  if ( xmlApi.isXpathString( fields ) ){
     fields = xmlApi.xpathToObj( fields, 1 );
     xmlOut = true;
  }
